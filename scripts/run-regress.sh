...
TESTS=("$@")
if [ ${#TESTS[@]} -eq 0 ]; then TESTS=(pg_tre scan_exact incremental p5_read planner planner_auto p6_safety utf8 tier3 dnf_resolution sparsemap_serialize multi_leaf similarity concurrently cardinality variable_width_bloom); fi
